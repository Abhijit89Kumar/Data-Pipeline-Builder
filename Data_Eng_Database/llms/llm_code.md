...
- `model_name`: Name of the Azure OpenAI deployment model to use (e.g., 'gpt-35-turbo', 'gpt-4', etc.)
...
- `logit_bias`: Bias for specific tokens (dict of token_id: bias)
- `logprobs`: Number of logprobs to return (int)
- `user`: Unique user identifier (string)
- `stop`: Sequences where API will stop generating (string or list of strings)
- `seed`: Random number generator seed (int)
- `suffix`: Suffix to append to the generated text (string)
- `azure_endpoint`: Azure OpenAI endpoint URL (string)
- `api_key`: Azure OpenAI API key (string)
- `api_version`: API version to use (string)
